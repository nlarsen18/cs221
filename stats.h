...
public:
    ...
    // its cartesian coordinates: X = 1*cos(h) and Y = 1*sin(h). 
    ...
    //helper function for filling in all the Sum 2D vectors
    void sumHelp(PNG & im, int p, int c);
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
    ...
    // helper function for getAvg, returns the average value of the channel
    // over the specified square by ul and lr. Basically does the 4-3-2+1 
    // calculation to stop doing it three times within getAvg.
    double helpGetAvg(pair<int,int> ul, pair<int,int> lr, vector< vector< double>> channel);

    //The following are helper functions for helpGetAvg. Which is called
    // depends on what shape the coordiantes ul and lr make (normal square,
    // split along y, etc ...)
    double helpAllNorm(pair<int,int> ul, pair<int,int> lr, vector< vector< double>> channel);

    double helpYNorm(pair<int,int> ul, pair<int,int> lr, vector< vector< double>> channel);

    double helpXNorm(pair<int,int> ul, pair<int,int> lr, vector< vector< double>> channel);

    double helpNoNorm(pair<int,int> ul, pair<int,int> lr, vector< vector< double>> channel);
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
    ...
    //The following are helper functions for buildHist when the square
    //is possibly split
    vector<int> helpHistAllN(pair<int,int> ul, pair<int,int> lr);

    vector<int> helpHistYN(pair<int,int> ul, pair<int,int> lr);

    vector<int> helpHistXN(pair<int,int> ul, pair<int,int> lr);

    vector<int> helpHistNoN(pair<int,int> ul, pair<int,int> lr);
